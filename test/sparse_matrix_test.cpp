...
TEST(SparseMatrix, ReturnsDefaultValue) {
  SparseMatrix<int>* intMatrix = new SparseMatrix<int>(10);
  ASSERT_EQ(10, intMatrix->get(0, 0));
  SparseMatrix<float>* floatMatrix = new SparseMatrix<float>(0.61f);
  ASSERT_EQ(0.61f, floatMatrix->get(0, 0));
}

TEST(SparseMatrix, ThrowsInvalidArgumentException) {
...
TEST(SparseMatrix, InsertionWorks) {
  SparseMatrix<int>* matrix = new SparseMatrix<int>(0);
  matrix->put(0, 1, 1);
